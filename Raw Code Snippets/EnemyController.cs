...
public class EnemyController : MonoBehaviour
{
    ...
    {
        // Null check!
        if (playerController.gameOver == 1 || playerController.gameOver == 2) 
        {
            return;
        }
        else
        {
            ...
            if (enemyHealth <= 0) {
                if(enemyMaterial == whiteGlow)
                {
                    bossdied = 1;
                    playerController.gameOver = 2;
                } else
                {
                    Destroy(gameObject);
                    pointValue = pointValue + playerController.currentLevel;
                    playerController.score += pointValue * OnClickMPK.moneyMulti;
                    if (playerController.playerHealth <= 10){
